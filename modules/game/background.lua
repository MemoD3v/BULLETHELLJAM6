...
function background.init()
    ...
    background.config = {
        ...
        -- Color settings
        colors = {
            {0.95, 0.34, 0.60, 0.8},  -- Pink
            {0.30, 0.65, 0.99, 0.8},  -- Blue
            {0.99, 0.80, 0.30, 0.8},  -- Yellow
            {0.50, 0.99, 0.60, 0.8}   -- Green
        },
        colorVariation = 0.1,
        ...
        -- Movement settings
        movementType = "flow",  -- "flow", "swirl", "random", "bounce", "noise", "orbit"
...
function background:_resetParticles()
    ...
    for i = 1, background.config.particleCount do
        ...
        local color = background.active and {
            ...
            baseColor[4]
        } or {1, 1, 1, 0.2}  -- Unactive mode color (faint white)
...
function background.activate() background.active = true; background:_resetParticles() end
...
function background.update(dt)
    ...
    for _, p in ipairs(background.particles) do
        ...
        elseif background.config.movementType == "swirl" then
            local centerX = background.config.swirlCenter.x * background.screenWidth
            local centerY = background.config.swirlCenter.y * background.screenHeight
        ...
        elseif background.config.movementType == "bounce" then
            -- Bounce off edges
            if p.x <= 0 or p.x >= background.screenWidth then
                p.angle = math.pi - p.angle
            end
            if p.y <= 0 or p.y >= background.screenHeight then
                p.angle = -p.angle
            end
        elseif background.config.movementType == "noise" then
            -- Noise-based movement
            local noiseX = love.math.noise(p.noiseOffsetX + background.time * background.config.noiseScale, background.noiseOffset)
            local noiseY = love.math.noise(p.noiseOffsetY + background.time * background.config.noiseScale, background.noiseOffset)
            p.angle = math.atan2(noiseY * 2 - 1, noiseX * 2 - 1)
        elseif background.config.movementType == "orbit" then
            -- Orbit around center
            p.orbitPhase = p.orbitPhase + dt * background.config.orbitSpeed
            local centerX = background.config.swirlCenter.x * background.screenWidth
            local centerY = background.config.swirlCenter.y * background.screenHeight
            p.x = centerX + math.cos(p.orbitPhase) * background.config.orbitRadius * (0.5 + 0.5 * love.math.noise(p.noiseOffsetX, background.time))
            p.y = centerY + math.sin(p.orbitPhase) * background.config.orbitRadius * (0.5 + 0.5 * love.math.noise(p.noiseOffsetY, background.time))
        end
        
        -- Move particle (unless in orbit mode which handles its own movement)
...
function background:_drawParticle(x, y, size, color, shape, rotation)
    ...
    elseif shape == "triangle" then
        ...
        love.graphics.translate(x, y)
        love.graphics.rotate(rotation)
        local h = size * math.sqrt(3)  -- Height of equilateral triangle
